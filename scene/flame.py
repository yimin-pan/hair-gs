...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Mask for FLAME
        mask_dict = np.load(config.flame_mask_path, allow_pickle=True, encoding="latin1")
        for key in mask_dict.keys():
            self.register_buffer(
                f"{key}_mask", torch.tensor(mask_dict[key], dtype=torch.long, device=self.v_template.device))
